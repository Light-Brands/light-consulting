 ...
 *
 * Uses YouTube IFrame API for dynamic video duration detection
...
export const AssessmentEducateStage: React.FC<AssessmentEducateStageProps> = ({
  ...
  const videoId = extractYouTubeVideoId(ASSESSMENT_CONFIG.vsl.url) || '';

  // Handle progress updates from YouTube player
  const handleProgress = useCallback(
    (progress: number) => {
      // Track maximum progress reached (prevents skipping)
      setMaxProgress((prev) => {
        const newMax = Math.max(prev, progress);
        if (newMax >= requiredPercentage && !canProceed) {
          setCanProceed(true);
        }
        return newMax;
      });
    },
    [requiredPercentage, canProceed]
  );
  ...
  const {
    ...
    videoId: hasStarted ? videoId : '',
    containerId,
    onProgress: handleProgress,
  });

  // Use max progress for display (prevents UI jumping when seeking backward)
  const displayProgress = Math.max(maxProgress, currentProgress);
  ...
  const handleContinue = useCallback(() => {
    if (canProceed) {
      onVSLComplete(displayProgress);
    }
  }, [canProceed, displayProgress, onVSLComplete]);

  // Calculate display duration - use actual duration if available, fallback to estimate
  const displayDuration =
    duration > 0 ? formatDurationShort(duration) : ASSESSMENT_CONFIG.vsl.estimatedDuration;
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="text-center space-y-2 sm:space-y-3 px-2">
        ...
        <p className="text-sm sm:text-base text-text-secondary max-w-xl mx-auto leading-relaxed">
          This video explains exactly what the AI Go/No-Go Assessment delivers, the investment
          required, and what to expect on your call.
          <strong className="text-text-primary block sm:inline mt-1 sm:mt-0">
            {' '}
            You must watch at least {requiredPercentage}% to continue.
          </strong>
        </p>
      </div>
      ...
      <div className="bg-depth-surface border border-depth-border rounded-xl overflow-hidden -mx-4 sm:mx-0">
        <div className="aspect-video relative bg-depth-base">
            ...
            >
              <div className="text-center space-y-3 sm:space-y-4">
                <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-radiance-gold/20 flex items-center justify-center mx-auto hover:bg-radiance-gold/30 active:scale-95 transition-all">
                  <svg
                    className="w-8 h-8 sm:w-10 sm:h-10 text-radiance-gold ml-1"
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                ...
                <p className="text-text-muted text-xs sm:text-sm">Duration: {displayDuration}</p>
              </div>
            </button>
          )}
          ...
          {hasStarted ? (
            ...
            // Thumbnail preview before starting
            <div className="absolute inset-0 bg-depth-base">
              {videoId && (
          ...
          {hasStarted && (
            <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 bg-gradient-to-t from-depth-base/95 to-transparent pointer-events-none z-10">
              <div className="space-y-1.5 sm:space-y-2">
                <div className="flex items-center justify-between text-xs sm:text-sm">
                  <span className="text-text-muted">Progress</span>
                  <span
                    className={`font-medium ${canProceed ? 'text-radiance-gold' : 'text-text-secondary'}`}
                  >
      ...
      {hasStarted && !isReady && (
        <div className="flex items-center justify-center gap-2 text-text-muted text-sm">
          <div className="w-4 h-4 border-2 border-radiance-gold/30 border-t-radiance-gold rounded-full animate-spin" />
          ...
          <div className="flex items-start gap-2 sm:gap-3">
            ...
            <div>
              <p className="text-text-secondary text-xs sm:text-sm leading-relaxed">
                <strong className="text-text-primary">Keep watching:</strong> You need to watch at
                least {requiredPercentage}% of this video before you can proceed. This ensures you
                understand the assessment process and investment.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start gap-2 sm:gap-3">
            ...
            <div className="space-y-2">
              <p className="text-text-secondary text-sm sm:text-base leading-relaxed">
                <strong className="text-text-primary">Video complete!</strong> You now understand
                what the AI Go/No-Go Assessment delivers.
              </p>
              <div className="bg-depth-base/50 rounded-lg p-3 sm:p-4 mt-2">
                ...
                <p className="text-text-muted text-xs sm:text-sm mt-1">
                  You&apos;re now ready to schedule your assessment call. The investment will be
      ...
      <div className="sticky bottom-0 bg-depth-base/95 backdrop-blur-sm -mx-4 px-4 py-3 sm:relative sm:mx-0 sm:px-0 sm:py-0 sm:bg-transparent sm:backdrop-blur-none border-t sm:border-t border-depth-border">
        <div className="flex items-center justify-between gap-3">
          <Button variant="ghost" onClick={onBack} className="min-h-[44px] text-sm sm:text-base">
          ...
          >
            {canProceed ? 'Schedule Your Call' : `${requiredPercentage - Math.round(displayProgress)}% more`}
          </Button>
        </div>
      </div>
    </div>
