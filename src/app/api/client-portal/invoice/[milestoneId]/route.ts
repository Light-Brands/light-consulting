/**
 * Client Invoice/Receipt API
 * Light Brand Consulting
 *
 * GET /api/client-portal/invoice/[milestoneId]
...
export async function GET(
  ...
  try {
    ...
    // Fetch milestone with proposal details
    ...
    // Verify the client owns this milestone
    const proposal = milestone.proposal as { id: string; client_email: string };
    if (proposal.client_email.toLowerCase() !== clientEmail) {
      return NextResponse.json(
        { error: 'Unauthorized' },
        { status: 403 }
      );
    ...
    if (milestone.payment_status !== 'paid' || !milestone.stripe_payment_intent_id) {
      return NextResponse.json(
        { error: 'Invoice not available - payment not yet completed' },
        { status: 400 }
      );
    }
