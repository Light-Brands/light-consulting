...
export default function BillingCenterPage() {
  ...
  const handlePayment = useCallback(async (payment: ClientPaymentItem) => {
    ...
    try {
      ...
      // Redirect to invoice URL (returned as both invoice_url and checkout_url)
      const invoiceUrl = result.invoice_url || result.checkout_url;
      if (invoiceUrl) {
                        ...
                        <div className="text-right flex-shrink-0">
                          ...
                          {payment.stripe_invoice_url ? (
                            <a
                              ...
                              className="inline-flex items-center gap-1 text-xs text-text-muted hover:text-radiance-gold mt-1 transition-colors"
                            >
                            ...
                            <a
                              href={`/api/client-portal/invoice/${payment.id}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 text-xs text-text-muted hover:text-radiance-gold mt-1 transition-colors"
                            >
        ...
        <div className="flex items-center gap-3 md:flex-shrink-0">
          ...
          {payment.stripe_invoice_url && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-2.5 text-text-secondary hover:text-text-primary border border-depth-border rounded-lg hover:bg-depth-elevated transition-colors"
              title="View Invoice"
            >
